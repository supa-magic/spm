...
import { error, info, item, success } from '@/utils/log'
  ...
  program
    ...
    .action(async (input: string) => {
      try {
        ...
        const tmpDir = join(tmpdir(), 'spm', skillset.name)

        info(`Downloading ${entries.length} file(s)...`)
        const results = await downloadEntries(entries, location)

        await Promise.all(
          results.map(async (result) => {
            const filePath = safePath(tmpDir, result.path)
